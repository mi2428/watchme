...
- **`network.wifi_gateway`:** Router used by `probe.gateway.path`, `probe.gateway.arp.resolve`, and `probe.gateway.icmp.echo`.
...
When BPF observes the outbound request and inbound reply packets, `probe.gateway.arp.resolve`, `probe.gateway.icmp.echo`, and the gateway duration/jitter metrics use BPF packet timestamps.
