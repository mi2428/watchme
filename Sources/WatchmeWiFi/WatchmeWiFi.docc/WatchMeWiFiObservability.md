...
- [Emitted spans](#emitted-spans)
  - [Root span](#root-span)
  - [Connectivity check spans](#connectivity-check-spans)
  - [Network attachment phase span](#network-attachment-phase-span)
...
- Root trace names such as `wifi.connectivity`, `wifi.join`, `wifi.roam`, `wifi.power.changed`, `wifi.link.changed`, and `wifi.network.attachment`.
- BPF packet spans for DHCPv4, IPv4 ARP, router solicitation/advertisement, and neighbor discovery timing.
- Internet DNS, ICMP, TCP connect, plain HTTP, and gateway probe metrics and spans for Wi-Fi-bound reachability.
...
DNS checks Wi-Fi-bound resolver reachability and produces the concrete remote addresses used by later probes.
ICMP checks address-family reachability without depending on HTTP service behavior.
TCP connect checks whether TCP/80 can complete a transport handshake before any HTTP payload is sent.
...
WatchMe expands configured hosts and address families into independent `target + family` probe paths.
Those paths run in parallel.
Within each path, checks run in this order: DNS, ICMP, TCP connect, then HTTP.
...
If correlation fails, WatchMe still emits the same span and metrics with callback or deadline timing, and carries the `timing_source` on the probe metadata and result counter metrics.
...
- **`--wifi.traces.interval`:** Connectivity trace interval in seconds. Default: `60`.
...
- **`--wifi.probe.internet.tcp`:** Boolean switch for internet TCP connect probes. Default: `true`.
...
- The local spool is bounded to 1000 pending files, 100 MiB, and seven days by default; oldest or expired payloads are dropped first.
- A single export attempts to replay at most 100 pending files before sending the current payload.
- In long-running mode, recovery is attempted on the next metrics interval, connectivity trace, or event-triggered export.
...
- after connectivity checks, so the latest internet DNS, ICMP, TCP, HTTP, and gateway probe samples are available to the OTel collector or backend.

Most metrics are gauges.
CoreWLAN event, snapshot change, and active probe result metrics are counters.
...
Active probe latest-value gauges use stable path labels.
Volatile details such as `remote_ip`, `outcome`, and `timing_source` are kept on `*_info` gauges for the latest probe metadata and `*_result_total` counters for historical outcomes.
...
| `watchme_wifi_probe_internet_path_success`, `watchme_wifi_probe_internet_path_duration_seconds`, `watchme_wifi_probe_internet_path_last_run_timestamp_seconds` | `interface`, `essid`, `bssid`, `target`, `family` | Wi-Fi-bound active internet path probe | Latest end-to-end internet path success, duration, and completion timestamp for one target/address-family path. |
| `watchme_wifi_probe_internet_path_info` | `interface`, `essid`, `bssid`, `target`, `family`, `remote_ip`, `outcome` | Wi-Fi-bound active internet path probe | Constant `1` info metric carrying latest path metadata. |
| `watchme_wifi_probe_internet_path_result_total` | `interface`, `essid`, `bssid`, `target`, `family`, `outcome` | Wi-Fi-bound active internet path probe | Count of internet path outcomes observed in this process. |
| `watchme_wifi_probe_internet_dns_success`, `watchme_wifi_probe_internet_dns_duration_seconds`, `watchme_wifi_probe_internet_dns_last_run_timestamp_seconds` | `interface`, `essid`, `bssid`, `target`, `family`, `resolver`, `transport`, `record_type` | Wi-Fi-bound active internet DNS probe | Latest DNS success, duration, and completion timestamp. |
| `watchme_wifi_probe_internet_dns_rcode` | `interface`, `essid`, `bssid`, `target`, `family`, `resolver`, `transport`, `record_type` | Wi-Fi-bound active internet DNS probe | DNS response code from the latest DNS probe when a response was parsed. |
| `watchme_wifi_probe_internet_dns_address_count` | `interface`, `essid`, `bssid`, `target`, `family`, `resolver`, `transport`, `record_type` | Wi-Fi-bound active internet DNS probe | Number of A or AAAA addresses returned by the latest DNS probe. |
| `watchme_wifi_probe_internet_dns_info` | `interface`, `essid`, `bssid`, `target`, `family`, `resolver`, `transport`, `record_type`, `outcome`, `timing_source` | Wi-Fi-bound active internet DNS probe | Constant `1` info metric carrying latest DNS metadata. |
| `watchme_wifi_probe_internet_dns_result_total` | `interface`, `essid`, `bssid`, `target`, `family`, `resolver`, `transport`, `record_type`, `outcome`, `timing_source` | Wi-Fi-bound active internet DNS probe | Count of DNS outcomes observed in this process. |
| `watchme_wifi_probe_internet_icmp_success`, `watchme_wifi_probe_internet_icmp_duration_seconds`, `watchme_wifi_probe_internet_icmp_last_run_timestamp_seconds` | `interface`, `essid`, `bssid`, `target`, `family` | Wi-Fi-bound active internet ICMP probe | Latest ICMP echo success, duration, and completion timestamp. |
| `watchme_wifi_probe_internet_icmp_info` | `interface`, `essid`, `bssid`, `target`, `family`, `remote_ip`, `outcome`, `timing_source` | Wi-Fi-bound active internet ICMP probe | Constant `1` info metric carrying latest ICMP metadata. |
| `watchme_wifi_probe_internet_icmp_result_total` | `interface`, `essid`, `bssid`, `target`, `family`, `outcome`, `timing_source` | Wi-Fi-bound active internet ICMP probe | Count of ICMP outcomes observed in this process. |
| `watchme_wifi_probe_internet_tcp_success`, `watchme_wifi_probe_internet_tcp_duration_seconds`, `watchme_wifi_probe_internet_tcp_last_run_timestamp_seconds` | `interface`, `essid`, `bssid`, `target`, `family`, `remote_port` | Wi-Fi-bound active internet TCP probe | Latest TCP connect success, duration, and completion timestamp. |
| `watchme_wifi_probe_internet_tcp_info` | `interface`, `essid`, `bssid`, `target`, `family`, `remote_port`, `remote_ip`, `outcome`, `timing_source` | Wi-Fi-bound active internet TCP probe | Constant `1` info metric carrying latest TCP metadata. |
| `watchme_wifi_probe_internet_tcp_result_total` | `interface`, `essid`, `bssid`, `target`, `family`, `remote_port`, `outcome`, `timing_source` | Wi-Fi-bound active internet TCP probe | Count of TCP outcomes observed in this process. |
| `watchme_wifi_probe_internet_http_success`, `watchme_wifi_probe_internet_http_duration_seconds`, `watchme_wifi_probe_internet_http_last_run_timestamp_seconds` | `interface`, `essid`, `bssid`, `target`, `family`, `scheme` | Wi-Fi-bound active internet plain HTTP probe | Latest plain HTTP HEAD success, duration, and completion timestamp. |
| `watchme_wifi_probe_internet_http_status_code` | `interface`, `essid`, `bssid`, `target`, `family`, `scheme` | Wi-Fi-bound active internet plain HTTP probe | HTTP status code from the latest probe when one was received. |
| `watchme_wifi_probe_internet_http_info` | `interface`, `essid`, `bssid`, `target`, `family`, `scheme`, `remote_ip`, `outcome`, `timing_source` | Wi-Fi-bound active internet plain HTTP probe | Constant `1` info metric carrying latest HTTP metadata. |
| `watchme_wifi_probe_internet_http_result_total` | `interface`, `essid`, `bssid`, `target`, `family`, `scheme`, `outcome`, `timing_source` | Wi-Fi-bound active internet plain HTTP probe | Count of HTTP outcomes observed in this process. |
| `watchme_wifi_probe_gateway_icmp_success`, `watchme_wifi_probe_gateway_icmp_duration_seconds`, `watchme_wifi_probe_gateway_icmp_probe_count`, `watchme_wifi_probe_gateway_icmp_reply_count`, `watchme_wifi_probe_gateway_icmp_loss_ratio`, `watchme_wifi_probe_gateway_icmp_jitter_seconds`, `watchme_wifi_probe_gateway_icmp_last_run_timestamp_seconds` | `interface`, `essid`, `bssid`, `gateway`, `family` | Wi-Fi-bound active gateway ICMP burst probe | Latest gateway ICMP burst success, timing, packet counts, loss, jitter, and completion timestamp. |
| `watchme_wifi_probe_gateway_icmp_info` | `interface`, `essid`, `bssid`, `gateway`, `family`, `outcome`, `timing_source` | Wi-Fi-bound active gateway ICMP burst probe | Constant `1` info metric carrying latest gateway ICMP metadata. |
| `watchme_wifi_probe_gateway_icmp_result_total` | `interface`, `essid`, `bssid`, `gateway`, `family`, `outcome`, `timing_source` | Wi-Fi-bound active gateway ICMP burst probe | Count of gateway ICMP outcomes observed in this process. |
| `watchme_wifi_probe_gateway_resolution_success`, `watchme_wifi_probe_gateway_resolution_duration_seconds`, `watchme_wifi_probe_gateway_resolution_last_run_timestamp_seconds` | `interface`, `essid`, `bssid`, `gateway`, `family`, `protocol` | Wi-Fi-bound active gateway ARP/NDP resolution probe | Latest gateway ARP or NDP resolution success, duration, and completion timestamp. |
| `watchme_wifi_probe_gateway_resolution_info` | `interface`, `essid`, `bssid`, `gateway`, `family`, `protocol`, `outcome`, `timing_source`, optional `gateway_hwaddr` | Wi-Fi-bound active gateway ARP/NDP resolution probe | Constant `1` info metric carrying latest gateway resolution metadata. |
| `watchme_wifi_probe_gateway_resolution_result_total` | `interface`, `essid`, `bssid`, `gateway`, `family`, `protocol`, `outcome`, `timing_source` | Wi-Fi-bound active gateway ARP/NDP resolution probe | Count of gateway ARP/NDP resolution outcomes observed in this process. |
...
- **`connectivity_check.skip_reason`:** Reason the connectivity check was skipped, such as `wifi_power_off`, `wifi_not_associated`, `wifi_interface_unknown`, or `wifi_dns_unavailable`.
- **`connectivity_check.readiness_wait_seconds`:** Seconds spent waiting for readiness before building the trace.
...
| --- | --- | --- | --- | --- |
| Derived root name such as `wifi.connectivity`, `wifi.join`, `wifi.roam`, `wifi.network.attachment` | None | Covers all child spans, including BPF spans that started before the trigger callback. | Always set to OK by exporter. | Common root tags listed above. |

### Connectivity check spans
...
- **`network.wifi_gateway`:** Router used by `probe.gateway.path.ipv4` or `probe.gateway.path.ipv6`, ARP/NDP resolution, `probe.gateway.icmp.burst`, and `probe.gateway.icmp.echo`.
...
- **`network.gateway_probe.outcome`:** Aggregated gateway burst outcome on path/burst spans, or the per-attempt outcome on `probe.gateway.icmp.echo`.
- **`network.gateway_probe.reachable`:** `true` when at least one gateway echo reply was observed on path/burst spans, or when the specific attempt received a reply on `probe.gateway.icmp.echo`.
...
### Network attachment phase span

This phase is emitted only when `PassivePacketStore.recentPacketSpans` returns at least one packet-derived span.

| Span name | Parent | Timing | Status | Tags |
| --- | --- | --- | --- | --- |
| `phase.network_attachment` | Root | Window from earliest packet span start to latest packet span end during Wi-Fi network attachment. | OK | `phase.name=network_attachment`, `phase.source=passive_bpf`, `phase.packet_span_count`, `span.source=watchme`, `otel.status_code=OK`. |

Packet-derived spans are recorded with `phase.network_attachment` as their logical parent and receive additional `wifi.essid` and `wifi.bssid` context.
...
| `packet.icmpv6.neighbor_solicitation_retry_gap` | Between consecutive Neighbor Solicitations for the same target address. | `packet.protocol=icmpv6`, `packet.event=neighbor_solicitation_retry_gap`, `icmpv6.nd.target_address`. |
| `packet.icmpv6.neighbor_solicitation_to_advertisement` | Latest Neighbor Solicitation before first Neighbor Advertisement for the same target address to that Advertisement. | `packet.protocol=icmpv6`, `packet.event=neighbor_solicitation_to_advertisement`, `icmpv6.nd.target_address`, `network.interface`, optional `icmpv6.nd.target_link_layer_address`, optional `icmpv6.nd.source_link_layer_address`. |

## Passive packet store behavior

`PassivePacketStore` is a rolling in-memory store for DHCP, ARP, and ICMPv6 observations, plus the active DNS/ICMP/TCP/HTTP registrations and matched packets used for active probe timing correlation.

- DHCP, ARP, ICMPv6, active DNS, active ICMP, active TCP, and active HTTP packet observations older than 600 seconds are pruned.
- Active probe registrations expire after the probe timeout plus one second.
- Active DNS, ICMP, TCP, and HTTP packet observations are retained only when they match a currently registered probe identity.
...
- TCP SYN/SYN-ACK/RST packets on port 80 that match a currently registered active TCP probe target address.
...
Active internet probes check the Wi-Fi path to internet hosts, not just general host reachability.
...
`--wifi.probe.internet.family=dual` expands each target into independent IPv4 and IPv6 probe paths.
`--wifi.probe.internet.family=ipv4` sends only A-record, IPv4 ICMP, IPv4 TCP, and IPv4 HTTP probes.
`--wifi.probe.internet.family=ipv6` sends only AAAA-record, IPv6 ICMP, IPv6 TCP, and IPv6 HTTP probes.
When DNS probing is enabled, each target/family path performs DNS first because hostname ICMP, TCP, and HTTP probes need a concrete address.
All target/family paths run in parallel, and each path records DNS -> ICMP -> TCP -> HTTP as child spans under one `probe.internet.path.ipv4` or `probe.internet.path.ipv6` span.
...
Only the first two Wi-Fi service DNS resolvers are probed to keep connectivity check cost bounded.
...
In that mode, ICMP, TCP, and HTTP probes still run, but hostname targets have no resolved remote address and produce `outcome=no_address`; literal IPv4 or IPv6 targets can still be probed for their matching address family.
...
TCP active probes connect to the resolved target address on TCP/80 through Network.framework with `requiredInterface` set to Wi-Fi.
Before opening the connection, WatchMe registers the target address, port, host, and interface with `PassivePacketStore`.
The BPF monitor stores only SYN/SYN-ACK/RST packets that match a registered active TCP probe.
When the outbound SYN and inbound SYN-ACK or RST are observed, `probe.internet.tcp.connect` and the TCP duration metric use BPF packet timestamps.
If packet correlation fails or BPF is disabled, the same span and metric fall back to Network.framework callback wall-clock timing.
...
When BPF observes the outbound request and inbound reply packets, `probe.gateway.arp.request_to_reply`, `probe.gateway.ndp.neighbor_solicitation_to_advertisement`, each `probe.gateway.icmp.echo`, and the gateway duration/jitter metrics use request-to-reply timing.
...
`probe.gateway.icmp.burst` is the burst window, so it includes the configured inter-attempt interval and is not a single RTT.
If packet correlation fails or BPF is disabled, the attempt span and metric fall back to wall-clock deadline timing.
